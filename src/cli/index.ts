...
const program = new Command('pw-eslint')
  ...
  .option(
    '--category <name>',
    'Run only rules in this category (repeatable)',
    collect,
    [] as string[]
  )
...
program
  ...
  .option('--no-color', 'Disable colored output')
  .action(async (ruleId: string | undefined, options: { list?: boolean; color: boolean }) => {
...
program.parseAsync(process.argv).catch((err: unknown) => {
  process.stderr.write(`[pw-eslint] Fatal error: ${String(err)}\n`);
  process.exit(2);
});
