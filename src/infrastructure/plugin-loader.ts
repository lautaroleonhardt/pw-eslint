  ...
  for (const filePath of files.sort()) {
    ...
    if (!exported || typeof exported !== 'object') {
      throw new PluginLoadError(
        filePath,
        'No default export found. Custom rules must export a RuleDefinition object as the default export.'
      );
    }
    ...
    if (obj['apiVersion'] !== SUPPORTED_API_VERSION) {
      ...
      throw new PluginLoadError(
        filePath,
        `Custom rule declares apiVersion ${String(obj['apiVersion'])}, but this CLI only supports apiVersion ${SUPPORTED_API_VERSION}. Rule is missing a valid "id" field.`
      );
    }
